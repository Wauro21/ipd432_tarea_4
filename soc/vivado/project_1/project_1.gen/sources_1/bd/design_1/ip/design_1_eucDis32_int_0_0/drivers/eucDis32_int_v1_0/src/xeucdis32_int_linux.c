...
typedef struct {
    ...
    XEucdis32_int_uio_map maps[ MAX_UIO_MAPS ];
} XEucdis32_int_uio_info;

/***************** Variable Definitions **************************************/
static XEucdis32_int_uio_info uio_info;
...
static int uio_info_read_name(XEucdis32_int_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XEucdis32_int_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XEucdis32_int_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XEucdis32_int_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XEucdis32_int_Initialize(XEucdis32_int *InstancePtr, const char* InstanceName) {
	XEucdis32_int_uio_info *InfoPtr = &uio_info;
